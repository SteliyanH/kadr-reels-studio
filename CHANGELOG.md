...
- **Schema v6 hands the composition to `KadrPersistence`.** The app carried a
  1,904-line hand-written mirror of kadr's DSL, with the failure mode every
  hand-written mirror has: kadr gains a field, nobody updates the mirror, and
  *nothing fails* — a field missing from both sides of a round-trip comparison
  compares equal. It had already cost three fields, and was still silently
  dropping `crop` and `quality`.

  App metadata — id, name, timestamps, timeline zoom, playhead mode, accent —
  stays here. The composition is now a `KadrDocument`, and inherits the
  package's completeness guard, which fails when kadr grows a field.

  v1–v5 projects migrate on read, in memory; the file is untouched until you
  save.

- **Images carry a token, not a re-encode.** `ProjectImageStore` is
  session-scoped so a photo import keeps its provenance from picker to save.
  Tokens are content-addressed, so re-saving an unchanged project produces
  identical bytes, and blobs the composition no longer references are pruned —
  which the old format never did.

- **Splitting a clip is `KadrUI.ClipSplitter`.** ~150 lines of arithmetic gone,
  and with it a bug: splitting an animated `TitleSequence` used to drop both
  keyframe tracks and the clip's start time.

- **The add-filter menu comes from `FilterKind`.** Eleven hand-written entries
  replaced by the catalogue kadr 1.0 ships, so a filter added to the engine
  appears here on its own.

- **Dependencies move to kadr 1.0** and the 1.0-compatible adapters: kadr-ui
  0.19.0, kadr-captions 0.12.0, kadr-photos 0.11.0, kadr-audio 0.7.0,
  kadr-persistence 0.3.0.

### Added

- **An app icon.** The set held only a `Contents.json` — no image at all, which
  is a hard submission blocker. The mark is a timeline: three clip bars, the top
  one cut, a playhead through the cut. Generated by `Tools/MakeIcon.swift`
  rather than checked in as an unexplained binary.

- **App Store metadata** under `fastlane/metadata/en-US`, and `PRIVACY.md`.

- **A crash-reporting toggle.** The privacy policy said reporting could be
  turned off; it could not — `CrashReporter` gated only on whether a Sentry DSN
  was present. Rather than weaken the sentence, Settings gained the switch. It
  reads through `object(forKey:)` so an absent key doesn't silently opt existing
  users out.

### Fixed

- **`AccentColor.colorset` was empty**, so the global accent fell back to system
  blue anywhere the Reel palette wasn't applied explicitly.
...
- **`ReelTheme.swift`** — token definitions (two palettes: print / studio), colour ramps (Neutral.n100–n900 / Accent.a100–a900 in OKLCH), spacing (4–32pt), typography (h1–h6 / body / caption / numeric), rules (2pt dividers), elevation (three levels, ink-tinted).
- **`ReelStyles.swift`** — reusable button styles (`ReelPrimaryButtonStyle` / `ReelSecondaryButtonStyle` / `ReelIconButtonStyle`), tag, card, rule, elevation, grayscale, label, segmented control, slider. All styles read `\.reelSurface` from the environment and adapt colours to the active palette.
- **Environment key.** `.reelSurface(_:)` modifier sets the palette (print / studio) once per screen; all descendant styles inherit.
...
- **`UIAppFonts` registration.** Declared in **`project.yml`** under the target's `info.properties` — `Info.plist` is generated by XcodeGen, so a direct plist edit is overwritten. Weight-trait resolution proven on-simulator (ExtraBold/SemiBold via family+trait path); guarded by `ReelTypographyTests`.
...
- **v0.5 `ColorPicker` replacement.** `setAccentColor(_:)` mutation and `ProjectDocument` schema v5 unchanged. UI narrowed: replaced free `ColorPicker` with `ReelSegmentedControl` over three accent ramp steps (`a500` / `a600` / `a700`) plus "System" option (clears to `nil`).
...
- **ReelStyles.swift `Body` → `StyleBody`.** Renamed nested view to avoid `ButtonStyle` associated-type witness collision (compile blocker, zero visual change).
